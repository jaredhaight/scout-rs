...
fn main() {
    ...
    let product_name = get_registry_value(
        HKEY_LOCAL_MACHINE,
        "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion",
        "ProductName",
    );
    let edition_id = get_registry_value(
        HKEY_LOCAL_MACHINE,
        "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion",
        "EditionID",
    );
    let release_id = get_registry_value(
        HKEY_LOCAL_MACHINE,
        "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion",
        "ReleaseId",
    );
    let build_branch = get_registry_value(
        HKEY_LOCAL_MACHINE,
        "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion",
        "BuildBranch",
    );
    let major_version = get_registry_value(
        HKEY_LOCAL_MACHINE,
        "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion",
        "CurrentMajorVersionNumber",
    );
    let current_version = get_registry_value(
        HKEY_LOCAL_MACHINE,
        "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion",
        "CurrentVersion",
    );
    ...
    for process_info in process_infos {
        if let Some(inner) = processes::lists::defensive_processes()
            .iter()
            .find(|&process| process.name.to_lowercase() == process_info.name.to_lowercase())
        {
            println!(
                "[Defensive Process] ({}) {} - {}",
                process_info.pid, process_info.name, inner.description
            )
        }
        if let Some(inner) = processes::lists::interesting_processes()
            .iter()
            .find(|&process| process.name.to_lowercase() == process_info.name.to_lowercase())
        {
            println!(
        ...
        if let Some(inner) = processes::lists::browser_processes()
            .iter()
            .find(|&process| process.name.to_lowercase() == process_info.name.to_lowercase())
        {
            println!(
    ...
    println!("\n\n================== POWERSHELL SETTINGS ==================");
    let powershell_2_version: String = get_registry_value(
        HKEY_LOCAL_MACHINE,
        "SOFTWARE\\Microsoft\\PowerShell\\1\\PowerShellEngine",
        "PowerShellVersion",
    );
    let powershell_3_version: String = get_registry_value(
        HKEY_LOCAL_MACHINE,
        "SOFTWARE\\Microsoft\\PowerShell\\3\\PowerShellEngine",
        "PowerShellVersion",
    );
    let transcription_value: String = get_registry_value(
        HKEY_LOCAL_MACHINE,
        "SOFTWARE\\Policies\\Microsoft\\Windows\\PowerShell\\Transcription",
        "EnableTranscripting",
    );
    let transcription_folder: String = get_registry_value(
        HKEY_LOCAL_MACHINE,
        "SOFTWARE\\Policies\\Microsoft\\Windows\\PowerShell\\Transcription",
        "OutputDirectory",
    );
    let module_logging_value: String = get_registry_value(
        HKEY_LOCAL_MACHINE,
        "SOFTWARE\\Policies\\Microsoft\\Windows\\PowerShell\\ModuleLogging",
        "EnableModuleLogging",
    );
    let module_logging_modules: String = get_registry_value(
        HKEY_LOCAL_MACHINE,
        "SOFTWARE\\Policies\\Microsoft\\Windows\\PowerShell\\ModuleLogging",
        "ModuleNames",
    );
    let script_block_logging_value: String = get_registry_value(
        HKEY_LOCAL_MACHINE,
    ...
    let av_wmi_con =
        WMIConnection::with_namespace_path("root\\SecurityCenter2", av_com_con.into()).unwrap();
    let results: Vec<HashMap<String, Variant>> = av_wmi_con
    ...
    println!("\n\n================== SYSMON CONFIG ===========================");
    let sysmon_hashing = get_registry_value(
        HKEY_LOCAL_MACHINE,
        "SYSTEM\\CurrentControlSet\\Services\\SysmonDrv\\Parameters",
        "HashingAlgorithm",
    );
    let sysmon_options = get_registry_value(
        HKEY_LOCAL_MACHINE,
        "SYSTEM\\CurrentControlSet\\Services\\SysmonDrv\\Parameters",
        "Options",
    );
